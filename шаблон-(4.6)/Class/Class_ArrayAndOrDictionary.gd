...
func fC_OvergrowthIf_Tr (vLD_FirstVariable: Dictionary, vLS_func: Dictionary, PathArray:= []) -> void:
	...
	for vLS_Key in vLD_FirstVariable:
		...
		var v__Result = Class_Help.FunctioСall(vLS_func_copy, PathArray)
...
func UniversalBypass (variables, funcparametrs: Dictionary, PathArray:= []):
	print(self.has_method("_С_ComparisonsOf2Variables_0101"))
	print("---UniversalBypass---")
	...
	print(keys)
	if keys == null:
		returnfunc1 = Class_Help.FunctioСall(funcparametrs, [type_id, variables1, PathArray, null])
	else:
		for elV in keys:
			var element = variables1[elV]
			PathArray.append(elV)
			returnfunc1 = Class_Help.FunctioСall(funcparametrs, [type_id , variables1, PathArray, elV])
			match returnfunc1:
				"Deeper":
...
func PassageAlongWay (Variables1, PathArray: Array, FuncParametrs:= {}):
	...
	for elPA in PathArray:
		if FuncParametrs != {}:
			...
			var ReturnFunctioСall = Class_Help.FunctioСall(FuncParametrsCopy,[VariablesReturn, Variables1, elPA])
			if ReturnFunctioСall != null and ReturnFunctioСall.has("VariablesReturn"):
				VariablesReturn = PassageAlongWay(VariablesReturn, ReturnFunctioСall ["VariablesReturn"])
			if ReturnFunctioСall != null and ReturnFunctioСall.has("flagPassageAlongWay"):
				flagPassageAlongWay = ReturnFunctioСall ["flagPassageAlongWay"]
...
func CheckingPath (PathArray: Array, FuncParametrs: Dictionary, Variables):
	...
	print (originalPathArray)
	while len(originalPathArray)-1 > 0:
		originalPathArray.pop_back()
		# Создаем копию словаря параметров, чтобы не изменять оригинальный
		var FuncParametrsCopy = FuncParametrs.duplicate()
		Class_Help.FunctioСall(FuncParametrsCopy, [PassageAlongWay(originalPathArray, Variables)])
...
##	2. Если 1. false то Проверка на то что оба контенеры если да то создаем список детей и вызываем UniversalBypass с _С_ComparisonsOf2Variables_0101 для обхода и сравнения
...
func fC_ComparisonsOf2Variables_0101 (Variables1, Variables2, ReturnDictionary):
	print ("\nVariables1: ",Variables1, ", Variables2: ",Variables2, ", ReturnDictionary: ",ReturnDictionary)
	...
	if typeof(Variables1) == typeof(Variables2):
		if Variables1 == Variables2:
			...
			print(self.has_method("_С_ComparisonsOf2Variables_0101"))
			UniversalBypass(ReturnDictionary, {"func": _С_ComparisonsOf2Variables_0101, "Parametrs": [Variables1, Variables2]})
		else:
			ReturnDictionary["result"] = "-"
			ReturnDictionary["Values"] = [Variables1, Variables2]
...
func _С_ComparisonsOf2Variables_0101 (InputArray: Array):
	if InputArray[5] != "result":
		...
		print ("Variables1: ",Variables1, ", Variables2: ",Variables2, ", ReturnDictionary: ",ReturnDictionary)
		#Сверка переменных
...
##Принцип работы:
##	зачем?
func _PAW_CO2V (VariablesReturn, Variables1, elPA):
	if elPA == "Child":
